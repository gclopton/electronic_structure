...
> [!question]
>
> (a) Write the electron kinetic energy operator in both SI-like and Hartree atomic units. What does each piece of the operator do, and why does the negative sign appear?
> (b) What property of a wavefunction determines how much kinetic energy the electron carries? Why does this make the kinetic energy impossible to express as a simple functional of the density alone?
> (c) How does the Kohn-Sham construction address the problem of computing the kinetic energy in DFT?
...
> [!question]
>
> (a) Write the external potential operator and explain the role of each piece. Why is this the only system-specific term in the Hamiltonian?
> (b) What is the general form $V_I(|\mathbf{r}_i - \mathbf{R}_I|)$, and why might it differ from the bare Coulomb potential in practical calculations?
...
> [!question]
>
> (a) Write the electron-electron interaction operator. What does each piece represent, and why is the factor of $\frac{1}{2}$ necessary?
> (b) Why does this term make the many-body problem fundamentally harder than a collection of independent particles?
> (c) What are the main strategies for approximating this term, and how do they differ from one another?
...
> [!question]
>
> (a) Write the nuclear kinetic energy operator and explain what each piece represents.
> (b) Why is this term much smaller than the electronic kinetic energy, and what approximation does neglecting it lead to?
> (c) Under what physical circumstances does this term become important again?
...
> [!question]
>
> (a) Write the ion-ion interaction energy and explain what each piece represents. Why does the factor of $\frac{1}{2}$ appear?
> (b) Why is this a scalar rather than an operator, and under what condition is it a constant?
> (c) This term does not affect the electronic eigenvalue problem. Why must it still be included when computing total energies?
...
> [!question]
>
> (a) Write the full Hamiltonian for a system of interacting electrons and nuclei. How many terms does it contain, and what physics does each encode?
> (b) Under the Born-Oppenheimer approximation, which terms survive in the electronic problem and which do not? What is the physical justification?
> (c) Of the remaining terms, which are universal and which depend on the specific system being studied?
...
> (b) What has been assumed about the nuclei to arrive at this Hamiltonian, and what justifies that assumption?
...
> (b) The many-body wavefunction $\Psi$ depends on $3N$ spatial coordinates, but the density depends on only 3. What operation reduces the dimensionality, and why does a prefactor of $N$ appear?
...
> (d) How does the density simplify when the electrons are non-interacting? Why is the resulting expression so much easier to evaluate?
...
| $f_i^\sigma$                                           | **Fermi-Dirac occupation**       | Scalar, $0 \leq f_i^\sigma \leq 1$            | —               | Probability of state $(i,\sigma)$ being occupied. Equals 0 or 1 at $T=0$; given by Fermi-Dirac at finite $T$.                                                                      |
...
> (c) The Hartree energy treats the charge density as a continuous classical distribution. What unphysical consequence does this have for an individual electron, and how do more advanced methods address it?
...
> [!question]
>
> (a) What does a projection operator do, and why is it needed?
> (b) What happens when a projection operator is applied twice?
> (c) How do projection operators appear in the density matrix?

> [!definition] Projection operator
>
> $$ \hat{P}_i^\sigma = |\psi_i^\sigma\rangle\langle\psi_i^\sigma| $$

Projection operator, $|\psi_i^\sigma\rangle\langle\psi_i^\sigma|$ — An operator that isolates the contribution of a single quantum state $|\psi_i^\sigma\rangle$ from the full Hilbert space. Because a quantum system can exist in a superposition of many states simultaneously, the projection operator provides the means to ask "how much of state $|\psi_i^\sigma\rangle$ is present in the system's current state?" — it keeps only the component along that state and discards everything orthogonal to it. When applied to an arbitrary state $|\phi\rangle$, it returns $|\psi_i^\sigma\rangle$ scaled by the overlap $\langle\psi_i^\sigma|\phi\rangle$. Applying it twice gives the same result as applying it once ($\hat{P}^2 = \hat{P}$), and its eigenvalues are 0 or 1 — a state is either entirely along $|\psi_i^\sigma\rangle$ or entirely orthogonal to it. In the density matrix, each term $f_i^\sigma|\psi_i^\sigma\rangle\langle\psi_i^\sigma|$ weights the projector by the Fermi-Dirac occupation $f_i^\sigma$, encoding the probability that orbital $i$ is occupied at thermal equilibrium; the full density matrix $\hat{\rho}$ is then a weighted sum of projectors — a complete census of which orbitals are occupied and by how much.

| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $\lvert\psi_i^\sigma\rangle\langle\psi_i^\sigma\rvert$ | Operator on single-particle Hilbert space | — | **Projection operator** | Isolates the contribution of state $\lvert\psi_i^\sigma\rangle$ from a superposition. Idempotent: $\hat{P}^2 = \hat{P}$. Eigenvalues are 0 or 1. |
| $\lvert\psi_i^\sigma\rangle$ | State vector | — | **Ket** | A single-particle quantum state (orbital $i$ with spin $\sigma$) in Dirac notation. |
| $\langle\psi_i^\sigma\rvert$ | Dual vector | — | **Bra** | The conjugate transpose of the ket. The product $\langle\psi_i^\sigma\rvert\phi\rangle$ gives the overlap (inner product) between two states. |
...
| $f_i^\sigma$ | Scalar, $0 \leq f_i^\sigma \leq 1$ | — | **Fermi-Dirac occupation** | Weight for orbital $i$ with spin $\sigma$. Given by the Fermi-Dirac distribution at finite temperature. |
...
Joint pair probability, $n(\mathbf{r},\sigma;\mathbf{r}',\sigma')$ — The probability density for simultaneously finding one electron of spin $\sigma$ at position $\mathbf{r}$ and a different electron of spin $\sigma'$ at position $\mathbf{r}'$. It is obtained by integrating the squared modulus of the many-body wavefunction over all electron coordinates except two — the two positions $\mathbf{r}$ and $\mathbf{r}'$ are left free because they are the two locations being probed — and multiplying by $N(N-1)$ because any of the $N$ electrons could be at $\mathbf{r}$ and any of the remaining $N-1$ could be at $\mathbf{r}'$. If electrons were completely independent, this joint probability would just be the product of individual spin-resolved densities $n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')$ (Entry 9, written here with spin as an argument rather than a superscript) — finding one electron at $\mathbf{r}$ would tell you nothing about whether another is at $\mathbf{r}'$. Real electrons are not independent, so one defines the pair correlation $\Delta n(\mathbf{r},\sigma;\mathbf{r}',\sigma') \equiv n(\mathbf{r},\sigma;\mathbf{r}',\sigma') - n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')$ as the deviation of the true joint probability from this uncorrelated baseline. Isolating $\Delta n$ is useful because it is exactly the quantity that determines the exchange-correlation energy — the central object DFT must approximate. Because two electrons that are far apart barely influence each other, $\Delta n$ decays to zero at large $|\mathbf{r} - \mathbf{r}'|$ — the correlation is short-ranged. This short-range character is the physical reason that local and semi-local density functional approximations (LDA, GGA) work as well as they do: a functional that depends only on the density in a small neighborhood of each point can still capture most of the correlation physics.
...
| $n(\mathbf{r},\sigma)$ | Scalar field | Bohr⁻³ | **Spin-resolved electron density** | Probability per unit volume of finding an electron of spin $\sigma$ at $\mathbf{r}$. For independent particles, $n(\mathbf{r},\sigma) = n^\sigma(\mathbf{r}) = \sum_i f_i^\sigma \lvert\psi_i^\sigma(\mathbf{r})\rvert^2$ (Entry 8). |
| $n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')$ | Product of densities | Bohr⁻⁶ | **Uncorrelated pair density** | Product of spin-resolved electron densities. What the joint probability would be if electrons were distributed independently. |
...
>
> $$ g(\mathbf{r},\sigma;\mathbf{r}',\sigma') = \frac{n(\mathbf{r},\sigma;\mathbf{r}',\sigma')}{n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')} $$
...
$$ g(\mathbf{r},\sigma;\mathbf{r}',\sigma') = 1 + \frac{\Delta n(\mathbf{r},\sigma;\mathbf{r}',\sigma')}{n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')} $$
...
For same-spin electrons at the same point, the Pauli exclusion principle requires $g(\mathbf{r},\sigma;\mathbf{r},\sigma) = 0$ — two electrons of the same spin can never occupy the same position.

At large separation $|\mathbf{r} - \mathbf{r}'| \to \infty$, $g \to 1$ because the pair correlation $\Delta n$ is short-ranged — distant electrons barely influence each other, so the joint probability approaches the uncorrelated product.
...
| $n(\mathbf{r},\sigma;\mathbf{r}',\sigma')$ | Two-point function | Bohr⁻⁶ | **Joint pair probability** | Probability density for simultaneously finding electrons of spins $\sigma$ and $\sigma'$ at positions $\mathbf{r}$ and $\mathbf{r}'$ (Entry 13). |
| $n(\mathbf{r},\sigma)\,n(\mathbf{r}',\sigma')$ | Product of densities | Bohr⁻⁶ | **Uncorrelated pair density** | Product of spin-resolved electron densities (Entry 8). The pair probability if electrons were distributed independently. |
| $\Delta n(\mathbf{r},\sigma;\mathbf{r}',\sigma')$ | Two-point function | Bohr⁻⁶ | **Pair correlation** | Deviation of the true joint probability from the uncorrelated baseline (Entry 13). Short-ranged. |
...
==Exchange hole==, $\Delta n_x$ — The depletion of same-spin electron density around each electron caused purely by the antisymmetry (Pauli exclusion) of the wavefunction, not by the Coulomb repulsion. The inner sum $\sum_i \psi_i^{\sigma*}(\mathbf{r})\,\psi_i^\sigma(\mathbf{r}')$ is the off-diagonal density matrix $\rho_\sigma(\mathbf{r},\mathbf{r}')$ — it measures how much the occupied same-spin orbitals overlap between the two points $\mathbf{r}$ and $\mathbf{r}'$, capturing the phase connection between them through all occupied states. Squaring its modulus gives the total overlap strength, and the minus sign makes the hole a depletion: the stronger the orbital overlap between two points, the more same-spin electron density is removed at $\mathbf{r}'$ when an electron is at $\mathbf{r}$. The Kronecker delta $\delta_{\sigma\sigma'}$ restricts the effect to same-spin pairs only — opposite-spin electrons are unaffected because their spin functions are orthogonal, so antisymmetry imposes no constraint on them. Three exact constraints govern the exchange hole: it is always non-positive ($\Delta n_x \leq 0$); at coincidence it exactly cancels the spin density ($\Delta n_x(\mathbf{r},\sigma;\mathbf{r},\sigma) = -n(\mathbf{r},\sigma)$, enforcing the Pauli exclusion principle); and it integrates over $\mathbf{r}'$ to exactly $-1$, meaning one electron's worth of charge is excluded from the vicinity of each electron — so the hole looks, from far away, like a missing electron.
...
> (b) What sum rules do the exchange hole and correlation hole individually satisfy, and what do they mean physically?
> (c) What does the correlation hole capture that the exchange hole does not, and why must they be treated separately?

> [!definition] Exchange-correlation hole (Eq. 3.58)
>
> $$ n_{\text{xc}}(\mathbf{r},\sigma;\mathbf{r}',\sigma') = n_x(\mathbf{r},\sigma;\mathbf{r}',\sigma') + n_c(\mathbf{r},\sigma;\mathbf{r}',\sigma') $$

Exchange-correlation hole, $n_{\text{xc}}$ — The total change in the density of surrounding electrons when there is an electron at position $\mathbf{r}$, combining the effects of both the Pauli exclusion principle and Coulomb repulsion. It answers the question: "given that an electron of spin $\sigma$ is at $\mathbf{r}$, how is the probability of finding another electron at $\mathbf{r}'$ modified compared to the average density?" The xc hole is the same object as the pair correlation $\Delta n$ from Entry 13, relabeled as $n_{\text{xc}}$ to emphasize its role in the exchange-correlation energy — the xc energy is exactly the Coulomb interaction of each electron with its xc hole, which is why modeling this hole accurately is the central problem of DFT. The decomposition into $n_x + n_c$ separates what a single Slater determinant gets right from what it misses: the exchange hole $n_x$ (Entry 15) can be computed exactly from the occupied orbitals, so it tells you what the Pauli principle alone accounts for; the correlation hole $n_c$ is everything beyond that, arising from the Coulomb interaction, and is the part that must be approximated. The two components satisfy distinct sum rules: $n_x$ integrates to exactly $-1$ (one electron's worth of charge is excluded), $n_c$ integrates to zero (it redistributes the hole but does not change its total weight), and the full $n_{\text{xc}}$ integrates to $-1$. These sum rules constrain the physics that any approximate functional must respect — the LDA satisfies all three exactly, which is one reason it performs as well as it does despite its simplicity: it gets the total excluded charge right even though it approximates the shape of the hole.

| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $n_{\text{xc}}$ | Two-point function | Bohr⁻³ | **Exchange-correlation hole** | Total modification of electron density around a reference electron due to exchange and correlation. Same object as the pair correlation $\Delta n$ (Entry 13). Integrates to $-1$. |
| $n_x$ | Two-point function | Bohr⁻³ | **Exchange hole** | Depletion from the Pauli principle alone (Entry 15). Always $\leq 0$. Same-spin only. Integrates to $-1$. Computable exactly from the occupied orbitals. |
| $n_c$ | Two-point function | Bohr⁻³ | **Correlation hole** | The part of the xc hole beyond exchange, arising from the Coulomb interaction. Can be positive or negative at different points. Integrates to zero — it redistributes the hole without changing its total weight. |
| $\Delta n$ | Two-point function | Bohr⁻³ | **Pair correlation** | Identical to $n_{\text{xc}}$ — the deviation of the true joint pair probability from the uncorrelated baseline (Entry 13). The notation $n_{\text{xc}}$ is used when emphasizing the role in the xc energy. |
...
> (b) How does the variational principle guarantee that any approximate wavefunction gives an upper bound to the true ground-state energy?
> (c) How does requiring stationarity of this functional lead to the Schrödinger equation?
...
Setting this variation to zero yields the time-independent Schrödinger equation $\hat{H}|\Psi\rangle = E|\Psi\rangle$ (Entry 19) as the Euler-Lagrange condition — the variational principle converts a differential equation into an optimization problem.
...
> (b) When you differentiate the total energy with respect to a nuclear position, not all terms survive. Why?
...
Force (Hellmann-Feynman) theorem — The statement that the force on nucleus $I$ is given entirely by classical electrostatics: the integral of the electron density weighted by the gradient of the nuclear potential, plus the Coulomb force from all other nuclei. The kinetic energy, exchange, and correlation do not appear as separate force terms — instead, they have already done their work by determining the self-consistent density $n(\mathbf{r})$ itself. Solving the Schrödinger (or Kohn-Sham) equations with all of these interactions included determines which orbitals are occupied and what spatial shape they have, and the density is built from those orbitals; the force formula then takes that fully self-consistent density as input and computes a purely electrostatic integral. The theorem relies on the ==variational principle==: at an eigenstate the energy is stationary with respect to variations of $\Psi$, so when you differentiate the total energy with respect to $\mathbf{R}_I$, only the explicit dependence of the Hamiltonian on $\mathbf{R}_I$ survives — the implicit dependence through $\Psi$ drops out. If the wavefunction is not fully variational — for instance, with an incomplete, position-dependent basis set — these implicit terms do not vanish and Pulay corrections must be added.
...
> (c) When differentiating the energy with respect to strain, not all terms in the derivative survive. Why?
...
Stress (generalized virial) theorem — The generalization of the force theorem (Entry 20) from displacing a single nucleus to uniformly deforming the entire system. Where the force theorem asks "how does the energy change if I nudge one nucleus?", the stress theorem asks "how does the energy change if I deform the simulation cell?" — where the deformation can be any combination of stretching, compressing, and shearing, all encoded in a single strain tensor. Strain is a uniform scaling that moves every coordinate in the system — electrons, nuclei, and cell vectors alike — so the stress tensor measures the internal resistance of the system to changes in its shape and volume. As with the force theorem, the variational principle ensures that when you differentiate the energy with respect to strain, only the explicit dependence of the Hamiltonian on the deformation survives — the implicit dependence through the wavefunction drops out. The diagonal components of the stress tensor give the pressure (resistance to uniform compression), while the off-diagonal components give shear stresses (resistance to shape changes at constant volume).
...
| $\alpha, \beta$ | Cartesian indices | — | **Spatial directions** | Each runs over $x, y, z$. Label the components of the stress and strain tensors. |
| $\delta_{\alpha\beta}$ | Kronecker delta | — | **Identity tensor** | Equals 1 if $\alpha = \beta$, 0 otherwise. The undeformed reference: $\epsilon_{\alpha\beta} = 0$ means no strain. |
| $k, k'$ | Particle indices | — | **Particle labels** | Sum over all particles (electrons and nuclei) in the system. |
| $m_k$ | Scalar | kg (or atomic units) | **Particle mass** | Mass of particle $k$. Electron mass $m_e$ for electrons, nuclear mass $M_I$ for nuclei. |
| $\nabla_{k\alpha}$ | Differential operator | Bohr⁻¹ | **Gradient component** | Derivative with respect to the $\alpha$-component of particle $k$'s position. The product $\nabla_{k\alpha}\nabla_{k\beta}$ forms the kinetic energy tensor. |
| $\mathbf{x}_{kk'} = \mathbf{r}_k - \mathbf{r}_{k'}$ | Vector | Bohr | **Pair separation vector** | Displacement from particle $k'$ to particle $k$. |
...
$$ \sigma_{\alpha\beta} = -\langle\Psi|\underbrace{\sum_k \frac{\hbar^2}{2m_k}\nabla_{k\alpha}\nabla_{k\beta}}_{\text{kinetic stress}} - \underbrace{\frac{1}{2}\sum_{k \neq k'}\frac{(\mathbf{x}_{kk'})_\alpha (\mathbf{x}_{kk'})_\beta}{x_{kk'}}\left(\frac{d}{dx_{kk'}}\hat{V}\right)}_{\text{interaction stress}}|\Psi\rangle $$

The first term is the kinetic stress — it measures how much momentum flux the particles carry in each direction. The prefactor $\hbar^2/2m_k$ is the same factor that appears in the kinetic energy operator $-\frac{\hbar^2}{2m}\nabla^2$, converting spatial curvature of the wavefunction into energy. The double gradient $\nabla_{k\alpha}\nabla_{k\beta}$ is the tensor generalization of the Laplacian: where $\nabla^2 = \nabla_x\nabla_x + \nabla_y\nabla_y + \nabla_z\nabla_z$ sums over all directions to give the total kinetic energy as a single number, keeping the indices $\alpha$ and $\beta$ separate tells you how the kinetic energy is distributed across directions — how much momentum is carried along $x$ vs $y$ vs $z$. Taking the trace ($\alpha = \beta$, summed) recovers the ordinary kinetic energy. Particles moving fast along the $\alpha$-direction push outward against the cell walls in that direction, resisting compression; this is the quantum-mechanical analog of ideal gas pressure, which comes entirely from particle motion. The second term is the interaction stress — it measures how much the Coulomb forces between particles pull inward or push outward along each pair of directions. The factor $(\mathbf{x}_{kk'})_\alpha (\mathbf{x}_{kk'})_\beta / x_{kk'}$ projects the radial force between each pair onto the $\alpha\beta$ component of the stress tensor, so a pair separated along $x$ contributes to $\sigma_{xx}$ but not to $\sigma_{yy}$. The factor of $\frac{1}{2}$ is a double-counting correction — the same one that appears in the electron-electron interaction (Entry 3) — because the sum over $k \neq k'$ counts each pair twice (once as $k,k'$ and once as $k',k$). The balance between the outward kinetic stress and the (typically inward) interaction stress determines whether the system wants to expand or contract — at equilibrium, the two balance and the net stress vanishes.
...
$$ E_{\text{potential}} = -2E_{\text{kinetic}}, \qquad E_{\text{total}} = -E_{\text{kinetic}} $$

This means the total energy of a bound Coulomb system at equilibrium is always negative and equals minus the kinetic energy — the potential energy is always twice as large in magnitude as the kinetic energy and opposite in sign.

| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $P$ | Scalar | Hartree/Bohr³ (or GPa) | **Pressure** | Isotropic part of the stress tensor: $P = -\frac{1}{3}\text{Tr}\,\sigma_{\alpha\beta}$. Positive means the system pushes outward (wants to expand). |
| $\Omega$ | Scalar | Bohr³ (or ų) | **System volume** | Volume of the simulation cell. The product $P\Omega$ has units of energy. |
| $E_{\text{kinetic}}$ | Scalar | Hartree | **Total kinetic energy** | Kinetic energy of all particles (electrons and nuclei). Scales as $L^{-2}$ under uniform dilation. |
| $E_{\text{potential}}$ | Scalar | Hartree | **Total potential energy** | All Coulomb interactions: electron-electron, electron-nuclear, and nuclear-nuclear. Scales as $L^{-1}$ under uniform dilation. |
| $3$ | Numerical prefactor | — | **Spatial dimension** | Arises from the trace over three Cartesian directions ($x, y, z$). In $d$ dimensions this would be $d$. |
| $2, 1$ | Numerical coefficients | — | **Scaling exponents** | Reflect how kinetic ($\propto L^{-2}$) and Coulomb potential ($\propto L^{-1}$) energies scale under uniform dilation. The 2-to-1 ratio is specific to the $1/r$ Coulomb interaction. |
...
> [!question]
>
> (a) What is the physical idea behind computing an energy difference by integrating over a continuous parameter?
...
Generalized force theorem and coupling constant integration — A technique for computing the energy difference between two states of a system by smoothly dialing a parameter $\lambda$ in the Hamiltonian from one value to another and accumulating the energy change along the way. The idea is: if you know how the energy responds to an infinitesimal change in $\lambda$ — given by the generalized Hellmann-Feynman derivative $\partial E/\partial\lambda = \langle\Psi_\lambda|\partial\hat{H}/\partial\lambda|\Psi_\lambda\rangle$ (Entry 20, Alternate Forms) — then you can add up all those infinitesimal changes to get a finite energy difference. The catch is that the expectation value must be evaluated with the eigenstate $\Psi_\lambda$ at each intermediate value of $\lambda$, so you need to know how the wavefunction evolves as the parameter changes — the system must remain in its ground state at every step (an "adiabatic" process). Despite requiring wavefunctions at unphysical intermediate coupling strengths that don't correspond to any real system, this formula is the conceptual foundation for the adiabatic connection formula for the exchange-correlation energy in DFT: it provides an exact expression for $E_{\text{xc}}$ that approximate functionals can be designed to satisfy.

| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $\lambda$ | Scalar parameter | — | **Coupling constant** | Any continuous parameter in the Hamiltonian. In the most important application, it scales the electron-electron interaction from 0 (noninteracting) to 1 (fully interacting). |
| $\lambda_1, \lambda_2$ | Scalar | — | **Integration limits** | The initial and final values of the coupling constant. The energy difference $\Delta E$ is computed between these two endpoints. |
| $\Psi_\lambda$ | $N$-electron eigenstate | — | **Adiabatically connected wavefunction** | The ground-state wavefunction at coupling strength $\lambda$. The system must remain in its ground state as $\lambda$ changes — this is the "adiabatic" assumption. Unphysical for $0 < \lambda < 1$. |
...
At $\lambda = 0$ the electrons do not interact and the wavefunction is a single Slater determinant of Kohn-Sham orbitals. At $\lambda = 1$ the electrons interact fully and the wavefunction is the true, correlated ground state. The integral smoothly connects these two limits, accumulating the energy cost of "turning on" the electron-electron repulsion.
...
This assigns occupation probability proportional to $e^{-\beta E_i}$ to each eigenstate $|\Psi_i\rangle$ — states with lower energy get exponentially more weight, with the sharpness controlled by $\beta$. At $T = 0$ ($\beta \to \infty$) only the ground state survives: $\hat{\rho} \to |\Psi_0\rangle\langle\Psi_0|$.

| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $F$ | Scalar | Hartree | **Helmholtz free energy** | $F = U - TS$. The thermodynamic potential minimized at fixed $T$, $V$, and $N$. Equilibrium is the state that best balances low energy against high entropy. |
| $\hat{\rho}$ | Operator | — | **Many-body density matrix** | Encodes the statistical mixture of quantum states. Positive semi-definite; $\text{Tr}\,\hat{\rho} = 1$. Not the same as the single-body density matrix in Entry 11 — this one acts on the full $N$-electron Hilbert space. |
| $\text{Tr}$ | Operation | — | **Trace** | Sums over all quantum states of the system: $\text{Tr}\,\hat{A} = \sum_i \langle\Psi_i\lvert\hat{A}\rvert\Psi_i\rangle$. Converts an operator into a number by adding up its diagonal matrix elements in any complete basis. |
| $\ln\hat{\rho}$ | Operator | — | **Matrix logarithm** | The operator whose exponential gives $\hat{\rho}$: $e^{\ln\hat{\rho}} = \hat{\rho}$. In the eigenbasis of $\hat{\rho}$, it simply takes the logarithm of each eigenvalue. Appears in the entropy because entropy measures the logarithm of the number of accessible states. |
| $e^{-\beta\hat{H}}$ | Operator | — | **Boltzmann operator** | Assigns exponential weight $e^{-\beta E_i}$ to each eigenstate $\lvert\Psi_i\rangle$. The fundamental object of quantum statistical mechanics — it encodes the thermal competition between energy and entropy. |
| $Z$ | Scalar | — | **Partition function** | Normalization constant ensuring $\text{Tr}\,\hat{\rho} = 1$. Connected to thermodynamics by $Z = e^{-\beta F}$. |
| $\beta = 1/(k_BT)$ | Scalar | Hartree⁻¹ | **Inverse temperature** | Controls the competition between energy and entropy. Large $\beta$ (low $T$): energy dominates. Small $\beta$ (high $T$): entropy dominates. |
...
>
> $$ -\varepsilon_i = E_{\text{HF}}^{\text{frozen}}(N-1, i) - E_{\text{HF}}(N) \qquad \text{(removal from occupied orbital } i\text{)} $$
> $$ -\varepsilon_a = E_{\text{HF}}(N) - E_{\text{HF}}^{\text{frozen}}(N+1, a) \qquad \text{(addition to empty orbital } a\text{)} $$
...
| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $\varepsilon_i$ | Scalar | Hartree (or eV) | **Occupied orbital eigenvalue** | Hartree-Fock eigenvalue of occupied orbital $i$. Under Koopmans' theorem, $-\varepsilon_i$ is the frozen-orbital ionization energy. |
| $\varepsilon_a$ | Scalar | Hartree (or eV) | **Unoccupied orbital eigenvalue** | Hartree-Fock eigenvalue of empty orbital $a$. Under Koopmans' theorem, $-\varepsilon_a$ is the frozen-orbital electron affinity. |
| $E_{\text{HF}}(N)$ | Scalar | Hartree | **$N$-electron Hartree-Fock energy** | Total Hartree-Fock energy of the $N$-electron system. |
...
> **Connections Forward:** Koopmans' theorem motivates the $\Delta$SCF method (Entry 34), which fixes the frozen-orbital limitation by computing $E(N) - E(N-1)$ with full self-consistent relaxation. In DFT, Janak's theorem (Ch 7) provides an analog relating eigenvalues to derivatives of the total energy with respect to occupation. The discrepancy between HF and DFT gaps is central to understanding the band gap problem.
...
| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $E$ | Scalar | Hartree | **Total energy** | The exact total electronic energy of the interacting system. |
| $\langle\hat{T}\rangle$ | Scalar | Hartree | **Interacting kinetic energy** | The expectation value of the kinetic energy operator in the true, correlated many-body wavefunction. This is *not* the same as the Kohn-Sham (non-interacting) kinetic energy $T_s$ — the difference $\langle\hat{T}\rangle - T_s$ is absorbed into the exchange-correlation energy in DFT. |
| $\langle\hat{V}_{\text{int}}\rangle$ | Scalar | Hartree | **Electron-electron repulsion energy** | The expectation value of the full quantum-mechanical electron-electron Coulomb interaction $\hat{V}_{\text{int}} = \frac{1}{2}\sum_{i \neq j} 1/\lvert\mathbf{r}_i - \mathbf{r}_j\rvert$ (Entry 3) in the true many-body wavefunction. Includes exchange and correlation effects. |
| $E_{\text{Hartree}}$ | Scalar | Hartree | **Hartree energy** | The classical mean-field approximation to $\langle\hat{V}_{\text{int}}\rangle$ (Entry 9). Treats the electron density as a smooth charge distribution and ignores all quantum correlations. |
| $\langle\hat{V}_{\text{int}}\rangle - E_{\text{Hartree}}$ | Scalar | Hartree | **Potential part of $E_{\text{xc}}$** | The difference between the true quantum electron-electron repulsion and its classical mean-field approximation. Short-ranged: the long-range $1/r$ tails cancel because both share the same average density at large separations, leaving only short-range exchange and correlation corrections. Always negative — exchange and correlation lower the energy below the mean-field estimate. |
...
> (c) What happens when $i = j$ in the direct and exchange sums, and why is this important?
...
Hartree-Fock total energy — The total energy of a system described by a single Slater determinant (Entry 10), expressed as a sum of four physically distinct contributions. The first line contains the single-particle terms: the kinetic energy and external potential for each occupied orbital, plus the ion-ion repulsion $E_{II}$ — these are the same terms you would have in a non-interacting calculation. The second line is the direct (Hartree) term: the classical Coulomb repulsion between the charge density $|\psi_i(\mathbf{r})|^2$ of orbital $i$ at point $\mathbf{r}$ and the charge density $|\psi_j(\mathbf{r}')|^2$ of orbital $j$ at point $\mathbf{r}'$ — each orbital's density stays at its own point, and you sum over all pairs including $i = j$. The third line is the exchange term, which has the same Coulomb integral but with the orbitals crossed over: instead of orbital $i$'s density at $\mathbf{r}$ and orbital $j$'s density at $\mathbf{r}'$, you have $\psi_i^*(\mathbf{r})\psi_j(\mathbf{r})$ at one point and $\psi_j^*(\mathbf{r}')\psi_i(\mathbf{r}')$ at the other — orbital $i$ starts at $\mathbf{r}$ and ends at $\mathbf{r}'$, while orbital $j$ does the reverse. This crossing is the "exchange," and it only occurs between same-spin pairs because opposite-spin orbitals are orthogonal in spin and contribute nothing. The exchange term is what makes the energy depend on more than just the density — it depends on which specific orbitals are occupied and how they overlap, making it inherently nonlocal. Including the self-term $i = j$ in the direct sum allows it to be written compactly as the Hartree energy of the total density $n(\mathbf{r})$; the corresponding $i = j$ term in the exchange then exactly cancels this spurious self-interaction, ensuring that each electron does not repel itself. This exact self-interaction cancellation is a defining feature of Hartree-Fock theory and is only approximately achieved by standard density functional approximations.
...
| $[\ldots]_{\text{HFA}}$ | Notation | — | **Hartree-Fock Approximation** | Indicates the enclosed expression is evaluated using the Hartree-Fock (single-determinant) wavefunction, not the true interacting wavefunction. This restriction is what makes the result the exchange energy $E_x$ alone rather than the full potential part of $E_{\text{xc}}$ (Entry 27) — the same difference $\langle\hat{V}_{\text{int}}\rangle - E_{\text{Hartree}}$ evaluated with the exact wavefunction includes correlation as well. |
| $\Delta n_x(\mathbf{r},\sigma;\mathbf{r}',\sigma')$ | Two-point function | Bohr⁻³ | **Exchange hole** | Depletion of same-spin electron density around each electron (Entry 15). Always $\leq 0$. Integrates to $-1$. |
| $1/\lvert\mathbf{r}-\mathbf{r}'\rvert$ | Coulomb kernel | Bohr⁻¹ | **Pair Coulomb weight** | Weights the exchange hole by inverse separation; determines the energy contribution of the hole. |
...
| $\hat{O}$ | Operator | — | **Single-body operator** | Any operator that acts on one electron at a time (e.g., kinetic energy, external potential). Two-body operators like $\hat{V}_{\text{int}}$ cannot be reduced this way. |
| $\psi_i^\sigma$ | Function | Bohr⁻³ᐟ² | **Single-particle orbital** | Eigenstate of the effective single-particle Hamiltonian with quantum numbers $i$ and spin $\sigma$. |
| $\sigma$ | Label | — | **Spin index** | Labels the spin channel ($\uparrow$ or $\downarrow$). |
| $T$ | Scalar | K (or eV via $k_B$) | **Temperature** | Electronic temperature controlling the smearing of Fermi-Dirac occupations. |
...
At $T = 0$, the Fermi-Dirac weights become step functions — $f_i^\sigma = 1$ for $\varepsilon_i^\sigma < \mu$ and $f_i^\sigma = 0$ for $\varepsilon_i^\sigma > \mu$ — and the sum counts each occupied eigenvalue exactly once.
...
| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $\hat{H}_{\text{full}}$ | Operator | Hartree | **Full Hamiltonian** | The complete Hamiltonian including both electronic and nuclear kinetic energy. |
| $\hat{H}_{\text{elec}}(\{\mathbf{R}_I\})$ | Operator | Hartree | **Electronic Hamiltonian** | The Hamiltonian with nuclear positions frozen — contains electron kinetic energy, electron-nuclear attraction, and electron-electron repulsion. Identical to $\hat{H}$ in Entry 7. |
| $\hat{T}_{\text{nuclei}}$ | Operator | Hartree | **Nuclear kinetic energy** | The term dropped by the Born-Oppenheimer approximation. Small because nuclear masses $M_I$ appear in the denominator. |
| $\{\mathbf{R}_I\}$ | Set of vectors | Bohr | **Nuclear positions** | Treated as fixed parameters (not quantum operators) within the BO approximation. |
...
| $V_{\text{ext}}(\mathbf{r})$ | Scalar field | Hartree | **External potential** | The nuclear attraction potential (and any applied fields). The only part of $V_{\text{eff}}$ that is the same in every formulation. |
...
| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $-\frac{1}{2}\nabla^2$ | Operator | Hartree | **Kinetic energy operator** | In atomic units ($\hbar = m_e = 1$), the same $-\frac{\hbar^2}{2m_e}\nabla^2$ from earlier entries. |
| $V_{\text{ext}}(\mathbf{r})$ | Scalar field | Hartree | **External potential** | Nuclear attraction. Same as in Entry 32. |
| $V_{\text{Hartree}}(\mathbf{r})$ | Scalar field | Hartree | **Hartree potential** | Classical electrostatic potential from total density. Includes self-interaction (Entry 9). |
| $\psi_j^{\sigma*}(\mathbf{r}')\psi_i^\sigma(\mathbf{r}')$ | Function | Bohr⁻³ | **Exchange charge density** | The "crossed" density where orbital $j$ and orbital $i$ swap positions. This is what makes the exchange operator nonlocal — it mixes different orbitals at different points. |
| $\sum_j$ | Sum | — | **Sum over occupied same-spin orbitals** | Runs over all occupied orbitals with the same spin $\sigma$ as orbital $i$. Exchange only acts between same-spin electrons. |
| $\varepsilon_i^\sigma$ | Scalar | Hartree | **Hartree-Fock eigenvalue** | Energy of orbital $i$. Equals the negative ionization energy under the frozen-orbital (Koopmans') assumption (Entry 25). |
...
$$\hat{H}_{\text{eff}}^{i}\psi_i^\sigma = \varepsilon_i^\sigma\psi_i^\sigma, \qquad \hat{V}_{\text{eff}}^{i,\sigma} = V_{\text{ext}} + V_{\text{Hartree}} + \hat{V}_x^{i,\sigma}$$
...
> (c) Why can DFT total energies give good ionization energies even though Kohn-Sham eigenvalues do not?
...
| **Symbol** | **Type** | **Units** | **Name** | **Description** |
|---|---|---|---|---|
| $E(N)$ | Scalar | Hartree | **Ground-state total energy** | The self-consistent total energy of the $N$-electron system. In DFT, this includes kinetic, Hartree, external, and xc contributions — all evaluated with relaxed orbitals. |
| $E(N-1)$ | Scalar | Hartree | **$(N-1)$-electron total energy** | Total energy after removing one electron from state $i$ and allowing all remaining orbitals to relax to self-consistency. |
| $E(N+1)$ | Scalar | Hartree | **$(N+1)$-electron total energy** | Total energy after adding one electron to state $a$ and relaxing all orbitals. |
| $E_{\text{removal}}$ | Scalar | Hartree (or eV) | **Removal energy (ionization energy)** | Energy cost to remove an electron. Positive for bound systems. Improves on Koopmans' $-\varepsilon_i$ (Entry 25) by including orbital relaxation. |
