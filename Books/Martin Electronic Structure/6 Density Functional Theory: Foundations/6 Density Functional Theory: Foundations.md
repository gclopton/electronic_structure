...
The original density functional theory of quantum systems is the ==Thomas-Fermi-Dirac approximation==, proposed in 1927 in separate papers by Thomas [327] and Fermi [328]. Although their approximation is not accurate enough for present-day electronic structure calculations, the approach illustrates the way density functional theory works. The approximation was to treat the kinetic energy of the system of electrons as an explicit functional of the density, idealized as noninteracting electrons in a homogeneous gas with density equal to the local density at any given point. Both Thomas and Fermi neglected exchange and correlation among the electrons; however, this was extended by Dirac [329] in 1930, who formulated the local approximation for exchange (==Dirac exchange==; see Sections 5.2 and 8.3) still in use today. This leads to the energy functional for electrons in an ==external potential== $V_{\text {ext }}(\mathbf{r})$ :
...
The ground state density and energy can be found by minimizing the functional $E[n]$ in Eq. (6.1) for all possible $n(\mathbf{r})$ subject to the ==particle number constraint==
...
where the Lagrange multiplier $\mu$ is the ==Fermi energy== (==chemical potential==). For small variations of the density $\delta n(\mathbf{r})$, the condition for a stationary point is ${ }^{1}$
...
Extensions to treat inhomogeneous systems more accurately require going beyond the local density approximation for the kinetic energy density. For example, the ==Weizsäcker correction== [330]:

> [!approximation] Weizsäcker gradient correction
>
> $$T_W[n] = \frac{1}{4}(\nabla n(\mathbf{r}))^{2} / n(\mathbf{r})$$

which is in essence a gradient correction in the same spirit as the gradient corrections in more modern density functionals in Section 8.5. In Section H. 1 it is shown that this can be viewed as the kinetic energy density of a system of bosons with density $n(\mathbf{r})$, and a more accurate expression for fermions always increases the kinetic energy at each point $\mathbf{r}$.
...
in an external potential $V_{\text {ext }}(\mathbf{r})$, including any problem of electrons and fixed nuclei, where the ==many-body Hamiltonian== can be written ${ }^{2}$
...
## 6.3.1 Proof of Theorem I: Density As a Basic Variable
...
where $E_{I I}$ is the interaction energy of the nuclei (see Eq. (3.2) and related discussion). The ==Hohenberg-Kohn energy functional== $F_{\mathrm{HK}}[n]$ defined in Eq. (6.12) includes all internal energies, kinetic and potential, of the interacting electron system,

> [!functional] Universal functional (Eq. 6.13)
>
> $$
...
The most relevant example for our purposes is a Zeeman term that is different for up and down spin fermions (i.e., a magnetic field that acts only on the spins, not on the orbital motion). This is in fact one of the important effects of an external magnetic field, so that this can be considered as a physically realistic approximation. Within this model, one can rigorously generalize all the above arguments to include two types of densities, the ==electron density== $n(\mathbf{r})=n(\mathbf{r}, \sigma=\uparrow)+n(\mathbf{r}, \sigma=\downarrow)$ and the ==spin density== $s(\mathbf{r})=n(\mathbf{r}, \sigma=\uparrow) n(\mathbf{r}, \sigma=\downarrow)$. This leads to an energy functional
...
## 6.5.1 ==Mermin functional==: Finite Temperature and Ensemble Density Functional Theory
...
## 6.5.3 Electric Fields and Polarization
...
## 6.6.1 Allowed Densities for Electrons
...
## 6.6.2 Properties Obeyed by the Exact Density Functional Theory
...
We wish to minimize the Thomas-Fermi-Dirac energy functional
...
subject to the particle number constraint $\int d^3r\, n(\mathbf{r}) = N$, where $C_1 = \frac{3}{10}(3\pi^2)^{2/3}$ and $C_2 = -\frac{3}{4}\!\left(\frac{3}{\pi}\right)^{\!1/3}$.

Following Eq. (3.10), we introduce a Lagrange multiplier $\mu$ and construct the unconstrained functional
...
where we use the shorthand $n = n(\mathbf{r})$ and $n' = n(\mathbf{r}')$.

Now consider a small variation $n(\mathbf{r}) \to n(\mathbf{r}) + \delta n(\mathbf{r})$. The stationarity condition requires that the first-order change $\delta\Omega_{\text{TF}} = \Omega_{\text{TF}}[n + \delta n] - \Omega_{\text{TF}}[n]$ vanish for all $\delta n$. We compute the first-order variation of each term.

##### Variation of the kinetic energy term

$$
\delta T_{\text{TF}} = C_1 \int d^3r\, \left[(n + \delta n)^{5/3} - n^{5/3}\right]
$$

Now expand explicitly to first order. Factor out $n^{5/3}$ and use $(1+x)^\alpha \approx 1 + \alpha x$ for small $x = \delta n/n$:
...
##### Variation of the external potential term

This term is already linear in $n$, so:
...
$$
\delta E_x = C_2 \int d^3r\, \left[(n + \delta n)^{4/3} - n^{4/3}\right]
$$
...
##### Variation of the Hartree term

$$
\delta E_{\text{Hartree}} = \frac{1}{2}\int d^3r\, d^3r'\, \frac{(n + \delta n)(n' + \delta n') - n\, n'}{|\mathbf{r} - \mathbf{r}'|}
$$
...
$$
V_{\text{Hartree}}(\mathbf{r}) = \int d^3r'\, \frac{n(\mathbf{r}')}{|\mathbf{r} - \mathbf{r}'|}
$$
...
$$
\delta\!\left(-\mu \int d^3r\, n\right) = -\mu \int d^3r\, \delta n(\mathbf{r})
$$

##### Assembling the total variation

Collecting all five contributions:

$$
\delta\Omega_{\text{TF}} = \int d^3r\, \left[\frac{5}{3}\, C_1\, n^{2/3} + V_{\text{ext}}(\mathbf{r}) + \frac{4}{3}\, C_2\, n^{1/3} + V_{\text{Hartree}}(\mathbf{r}) - \mu\right] \delta n(\mathbf{r})
$$

##### Applying the fundamental lemma of the calculus of variations

The condition $\delta\Omega_{\text{TF}} = 0$ must hold for *all* variations $\delta n(\mathbf{r})$. By the fundamental lemma of the calculus of variations, this is possible if and only if the bracketed expression vanishes identically at every point $\mathbf{r}$:

$$
\frac{5}{3}\, C_1\, n(\mathbf{r})^{2/3} + V_{\text{ext}}(\mathbf{r}) + \frac{4}{3}\, C_2\, n(\mathbf{r})^{1/3} + V_{\text{Hartree}}(\mathbf{r}) = \mu
$$

##### Simplifying the kinetic prefactor

Substitute $C_1 = \frac{3}{10}(3\pi^2)^{2/3}$:

$$
\frac{5}{3} \cdot C_1 = \frac{5}{3} \cdot \frac{3}{10}(3\pi^2)^{2/3} = \frac{5 \cdot 3}{3 \cdot 10}(3\pi^2)^{2/3} = \frac{15}{30}(3\pi^2)^{2/3} = \frac{1}{2}(3\pi^2)^{2/3}
$$

##### Defining the total effective potential

Define the local exchange potential

$$
V_x(\mathbf{r}) = \frac{4}{3}\, C_2\, n(\mathbf{r})^{1/3}
$$

and the total effective potential

$$
V(\mathbf{r}) = V_{\text{ext}}(\mathbf{r}) + V_{\text{Hartree}}(\mathbf{r}) + V_x(\mathbf{r})
$$

##### The Thomas-Fermi equation (Eq. 6.5)
...
which is Eq. (6.5). $\quad\blacksquare$

This is a self-consistent equation: $n(\mathbf{r})$ determines $V_{\text{Hartree}}$ and $V_x$, which contribute to $V(\mathbf{r})$, which in turn determines $n(\mathbf{r})$ through the equation above. The first term, $\frac{1}{2}(3\pi^2 n)^{2/3} = \frac{\hbar^2 k_F^2}{2m_e}$, is the local Fermi kinetic energy — the kinetic energy of the fastest electron in a uniform gas of density $n(\mathbf{r})$. The equation therefore says: at every point, the local Fermi kinetic energy plus the total potential energy equals the constant $\mu$. In regions where $V(\mathbf{r}) > \mu$, the density must vanish (classically forbidden). The Lagrange multiplier $\mu$ is the chemical potential — the energy cost of adding one electron to the system — and at zero temperature it equals the Fermi energy.
